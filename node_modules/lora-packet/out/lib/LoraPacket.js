...
var Constants;
(function (Constants) {
    Constants[Constants["FCTRL_ADR"] = 128] = "FCTRL_ADR";
    Constants[Constants["FCTRL_ADRACKREQ"] = 64] = "FCTRL_ADRACKREQ";
    Constants[Constants["FCTRL_ACK"] = 32] = "FCTRL_ACK";
    Constants[Constants["FCTRL_FPENDING"] = 16] = "FCTRL_FPENDING";
    Constants[Constants["DLSETTINGS_RXONEDROFFSET_MASK"] = 112] = "DLSETTINGS_RXONEDROFFSET_MASK";
    Constants[Constants["DLSETTINGS_RXONEDROFFSET_POS"] = 4] = "DLSETTINGS_RXONEDROFFSET_POS";
    Constants[Constants["DLSETTINGS_RXTWODATARATE_MASK"] = 15] = "DLSETTINGS_RXTWODATARATE_MASK";
    Constants[Constants["DLSETTINGS_RXTWODATARATE_POS"] = 0] = "DLSETTINGS_RXTWODATARATE_POS";
    Constants[Constants["DLSETTINGS_OPTNEG_MASK"] = 128] = "DLSETTINGS_OPTNEG_MASK";
    Constants[Constants["DLSETTINGS_OPTNEG_POS"] = 7] = "DLSETTINGS_OPTNEG_POS";
    Constants[Constants["RXDELAY_DEL_MASK"] = 15] = "RXDELAY_DEL_MASK";
    Constants[Constants["RXDELAY_DEL_POS"] = 0] = "RXDELAY_DEL_POS";
})(Constants || (Constants = {}));
...
class LoraPacket {
    ...
    _initfromWire(contents) {
        ...
        if (mtype == MType.JOIN_REQUEST) {
            this.AppEUI = util_1.reverseBuffer(incoming.slice(1, 1 + 8));
            this.DevEUI = util_1.reverseBuffer(incoming.slice(9, 9 + 8));
            this.DevNonce = util_1.reverseBuffer(incoming.slice(17, 17 + 2));
        }
        else if (mtype == MType.JOIN_ACCEPT) {
            this.AppNonce = util_1.reverseBuffer(incoming.slice(1, 1 + 3));
            this.NetID = util_1.reverseBuffer(incoming.slice(4, 4 + 3));
            this.DevAddr = util_1.reverseBuffer(incoming.slice(7, 7 + 4));
            ...
            if (this.RejoinType[0] === 0 || this.RejoinType[0] === 2) {
                this.NetID = util_1.reverseBuffer(incoming.slice(2, 2 + 3));
                this.DevEUI = util_1.reverseBuffer(incoming.slice(5, 5 + 8));
                this.RJCount0 = util_1.reverseBuffer(incoming.slice(13, 13 + 2));
            }
            else if (this.RejoinType[0] === 1) {
                this.JoinEUI = util_1.reverseBuffer(incoming.slice(2, 2 + 8));
                this.DevEUI = util_1.reverseBuffer(incoming.slice(10, 10 + 8));
                this.RJCount1 = util_1.reverseBuffer(incoming.slice(13, 13 + 2));
            }
        }
            ...
            this.DevAddr = util_1.reverseBuffer(this.FHDR.slice(0, 4));
            this.FCnt = util_1.reverseBuffer(this.FHDR.slice(5, 7));
    ...
    isDataMessage() {
        const mtype = this._getMType();
        if (mtype >= MType.UNCONFIRMED_DATA_UP && mtype <= MType.CONFIRMED_DATA_DOWN)
            return true;
        return false;
    }
    isConfirmed() {
        const mtype = this._getMType();
        if (mtype === MType.CONFIRMED_DATA_DOWN || mtype === MType.CONFIRMED_DATA_UP)
            return true;
        return false;
    }
    /**
    ...
    getDLSettingsRxOneDRoffset() {
        if (!this.DLSettings)
            return null;
        return ((this.DLSettings.readUInt8(0) & Constants.DLSETTINGS_RXONEDROFFSET_MASK) >> Constants.DLSETTINGS_RXONEDROFFSET_POS);
    }
    /**
    ...
    getDLSettingsRxTwoDataRate() {
        if (!this.DLSettings)
            return null;
        return ((this.DLSettings.readUInt8(0) & Constants.DLSETTINGS_RXTWODATARATE_MASK) >> Constants.DLSETTINGS_RXTWODATARATE_POS);
    }
    /**
